...
class gameLogic:
    ...
    def assignIT(self, taggedPlayer=None, firstRound=False):
        ...
        if firstRound:
            ...
            if taggedPlayer: 
                ...
                numOfIT = self.updateCount(self, self.it_player)
                print(f"Player {self.it_player} remains IT and has been tagged {numOfIT} times.")
    ...
    def playerSelectBox(self, playerID, row, col):
        ...
        with self.gridLock:
            #prevent same player from selecting more than one box
            if playerID in self.boxSelected:
                print(f"Player {playerID} has already made selection.")
                return False
    ...
    def itSelectBox(self, row, col):
        ...
        with self.gridLock:
            if self.it_selected:
                print("You have already selected a box.")
                return False
            
            self.grid[row][col] = self.it_player
            self.it_selected = True
